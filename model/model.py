...
class QueryExecutor(nn.Module, core.Configurable):
    ...
    基于概率逻辑的查询推理，支持交(conjunction)、并(disjunction)、否(negation)、投影(projection)四种算子。
    
    理论基础：
    - 交集：概率乘积 P(A ∩ B) = P(A) × P(B)（假设独立性）
    - 并集：概率并集 P(A ∪ B) = P(A) + P(B) - P(A) × P(B)
    - 否定：补集 P(¬A) = 1 - P(A)
    """

    stack_size = 8  # 从10减少到8，V21最复杂结构需要约6-7层
    ...
    def execute(self, graph, query, all_loss=None, metric=None):
        ...
        修复说明：
        原来的代码有一个bug：projection只在没有其他操作时才执行。
        这会导致当batch中有不同结构的查询时，某些查询的projection被跳过，
        从而导致IP不同步，最终引发Stack underflow错误。
        
        修复方案：让projection可以和其他操作同时执行。
        """
        ...
        while not op.is_stop().all():
            ...
            # 修复：移除条件限制，让projection可以和其他操作同时执行
            if is_projection.any():
    ...
    def disjunction(self, x, y):
        ...
        理论基础：P(A ∪ B) = P(A) + P(B) - P(A ∩ B) = P(A) + P(B) - P(A) × P(B)
        """
        return x + y - x * y
