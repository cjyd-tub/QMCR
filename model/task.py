...
class LogicalQuery(tasks.Task, core.Configurable):
    ...
    def __init__(self, model, dataset="PWA", criterion="bce", metric=("mrr",), adversarial_temperature=0.1,
                 sample_weight=False):
    ...
    def evaluate(self, pred, target):
        ...
        for _metric in self.metric:
            if _metric.startswith("mrr@"):
                ...
                threshold = int(_metric[5:])
                query_score_list = []
                # Pre-compute log2 values to avoid creating tensors in the loop
                log2_vals = torch.log2(torch.arange(2, threshold + 2, dtype=torch.float32, device=self.device))

                for i in range(0, len(query_list)):
                    cur_pred = pred_list[i][:threshold]
                    cur_ans = set(ans_list[i])  # Convert to set for faster lookup
                    dcg = 0.0

                    # Calculate DCG
                    for j, item in enumerate(cur_pred):
                        if item in cur_ans:
                            # Using 1.0 / log2(j+2) for relevant items (binary relevance)
                            if j < len(log2_vals):
                                dcg += 1.0 / log2_vals[j]

                    # Calculate IDCG
                    idcg = 0.0
                    num_relevant = min(len(cur_ans), threshold)
                    for j in range(num_relevant):
                        if j < len(log2_vals):
                            idcg += 1.0 / log2_vals[j]

                    # Calculate NDCG
                    ratio = dcg / idcg if idcg > 0 else 0.0
                    query_score_list.append(ratio)

                query_score = torch.tensor(query_score_list, device=self.device)
                type_score = scatter_mean(query_score, type, dim_size=len(self.id2type))
            else:
